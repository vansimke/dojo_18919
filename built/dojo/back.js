//>>built
define("dojo/back", "./_base/config ./_base/lang ./sniff ./dom ./dom-construct ./_base/window require".split(" "), function(k, C, m, D, E, F, x) {
  function f() {
    var a = c.pop();
    if (a) {
      var b = c[c.length - 1];
  ...
  d.addToHistory = function(a) {
    e = [];
    var b = null, d = null;
    l || (k.useXDomain && !k.dojoIframeHistoryUrl && console.warn("dojo/back: When using cross-domain Dojo builds, please save iframe_history.html to your domain and set djConfig.dojoIframeHistoryUrl to the path on your domain to iframe_history.html"), l = window.frames.dj_history);
    w || (w = E.create("a", {style:{display:"none"}}, F.body()));
    if (a.changeUrl) {
      b = "" + (!0 !== a.changeUrl ? a.changeUrl : (new Date).getTime());
      if (0 == c.length && n.urlHash == b) {
        n = p(d, a, b);
        return;
      }
      if (0 < c.length && c[c.length - 1].urlHash == b) {
        c[c.length - 1] = p(d, a, b);
        return;
      }
      u = !0;
      setTimeout(function() {
      ...
      w.href = b;
      if (m("ie")) {
        var d = y(), f = a.back || a.backButton || a.handle, g = function(a) {
  ...
  d._iframeLoaded = function(a, b) {
    var d = r(b.href);
    null == d ? 1 == c.length && f() : t ? t = !1 : 2 <= c.length && d == r(c[c.length - 2].url) ? f() : 0 < e.length && d == r(e[e.length - 1].url) && h();
  };
  return d;
});
