...
define("dojo/fx/easing", ["../_base/lang"], function(d) {
  var b = {linear:function(a) {
    ...
    a *= 2;
    return 1 > a ? Math.pow(a, 4) / 2 : -.5 * (Math.pow(a - 2, 4) - 2);
  }, quintIn:function(a) {
    return Math.pow(a, 5);
  }, quintOut:function(a) {
    ...
    return -1 * Math.cos(Math.PI / 2 * a) + 1;
  }, sineOut:function(a) {
    ...
    a *= 2;
    return 1 > a ? -.5 * (Math.sqrt(1 - Math.pow(a, 2)) - 1) : .5 * (Math.sqrt(1 - Math.pow(a - 2, 2)) + 1);
  }, backIn:function(a) {
    ...
    --a;
    return -1 * Math.pow(2, 10 * a) * Math.sin(2 * (a - .075) * Math.PI / .3);
    ...
    var b = .3 * 1.5, c = b / 4;
    if (1 > a) {
      return --a, -.5 * Math.pow(2, 10 * a) * Math.sin(2 * (a - c) * Math.PI / b);
    }
    --a;
    return .5 * Math.pow(2, -10 * a) * Math.sin(2 * (a - c) * Math.PI / b) + 1;
  }, bounceIn:function(a) {
    return 1 - b.bounceOut(1 - a);
  }, bounceOut:function(a) {
    return a < 1 / 2.75 ? 7.5625 * Math.pow(a, 2) : a < 2 / 2.75 ? 7.5625 * Math.pow(a - 1.5 / 2.75, 2) + .75 : a < 2.5 / 2.75 ? 7.5625 * Math.pow(a - 2.25 / 2.75, 2) + .9375 : 7.5625 * Math.pow(a - 2.625 / 2.75, 2) + .984375;
  }, bounceInOut:function(a) {
    return .5 > a ? b.bounceIn(2 * a) / 2 : b.bounceOut(2 * a - 1) / 2 + .5;
  }};
