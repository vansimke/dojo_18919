//>>built
define("dijit/Editor", "require dojo/_base/array dojo/_base/declare dojo/Deferred dojo/i18n dojo/dom-attr dojo/dom-class dojo/dom-geometry dojo/dom-style dojo/keys dojo/_base/lang dojo/sniff dojo/string dojo/topic ./_Container ./Toolbar ./ToolbarSeparator ./layout/_LayoutWidget ./form/ToggleButton ./_editor/_Plugin ./_editor/plugins/EnterKeyHandling ./_editor/html ./_editor/range ./_editor/RichText ./main dojo/i18n!./_editor/nls/commands".split(" "), function(w, l, q, x, y, z, t, u, r, d, h, g, A, 
B, J, C, D, E, F, m, G, v, f, H, I) {
  function e(a) {
    return new m({command:a.name});
  }
  function n(a) {
    return new m({buttonClass:F, command:a.name});
  }
  q = q("dijit.Editor", H, {plugins:null, extraPlugins:null, constructor:function() {
    h.isArray(this.plugins) || (this.plugins = ["undo", "redo", "|", "cut", "copy", "paste", "|", "bold", "italic", "underline", "strikethrough", "|", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "|", "justifyLeft", "justifyRight", "justifyCenter", "justifyFull", G]);
    this._plugins = [];
    ...
    this.commands = y.getLocalization("dijit._editor", "commands", this.lang);
    g("webkit") && r.set(this.domNode, "KhtmlUserSelect", "none");
  }, startup:function() {
    this.inherited(arguments);
    this.toolbar || (this.toolbar = new C({ownerDocument:this.ownerDocument, dir:this.dir, lang:this.lang, "aria-label":this.id}), this.header.appendChild(this.toolbar.domNode));
    l.forEach(this.plugins, this.addPlugin, this);
    this.setValueDeferred.resolve(!0);
    t.add(this.iframe.parentNode, "dijitEditorIFrameContainer");
    t.add(this.iframe, "dijitEditorIFrame");
    z.set(this.iframe, "allowTransparency", !0);
    ...
    if (!c.setEditor) {
      ...
      c.name && (m.registry[c.name] ? p.plugin = m.registry[c.name](c) : B.publish(I._scopeName + ".Editor.getPlugin", p));
      if (!p.plugin) {
        try {
          var d = c.ctor || h.getObject(c.name) || w(c.name);
          d && (p.plugin = new d(c));
        } catch (e) {
    ...
    var a = this._contentBox.h - (this.getHeaderHeight() + this.getFooterHeight() + u.getPadBorderExtents(this.iframe.parentNode).h + u.getMarginExtents(this.iframe.parentNode).h);
    ...
    try {
      ...
      b = A.substitute, alert(b(this.commands.systemShortcut, [this.commands[a], b(this.commands[g("mac") ? "appleKey" : "ctrlKey"], [{cut:"X", copy:"C", paste:"V"}[a]])])), b = !1;
    }
    return b;
    ...
    a = a.isCollapsed;
    var d, e, k;
    c && (9 > g("ie") || 9 === g("ie") && g("quirks") ? h.isArray(c) ? (b = [], l.forEach(c, function(a) {
      b.push(f.getNode(a, this.editNode));
    }, this), this.selection.moveToBookmark({mark:b, isCollapsed:a})) : c.startContainer && c.endContainer && (k = f.getSelection(this.window)) && k.removeAllRanges && (k.removeAllRanges(), a = f.create(this.window), d = f.getNode(c.startContainer, this.editNode), e = f.getNode(c.endContainer, this.editNode), d && e && (a.setStart(d, c.startOffset), a.setEnd(e, c.endOffset), k.addRange(a))) : (k = f.getSelection(this.window)) && k.removeAllRanges && (k.removeAllRanges(), a = f.create(this.window), 
    d = f.getNode(c.startContainer, this.editNode), e = f.getNode(c.endContainer, this.editNode), d && e && (a.setStart(d, c.startOffset), a.setEnd(e, c.endOffset), k.addRange(a))));
    ...
    0 === this._steps.length && this._steps.push({text:v.getChildrenHtml(this.editNode), bookmark:this._getBookmark()});
  ...
  h.mixin(m.registry, {undo:e, redo:e, cut:e, copy:e, paste:e, insertOrderedList:e, insertUnorderedList:e, indent:e, outdent:e, justifyCenter:e, justifyFull:e, justifyLeft:e, justifyRight:e, "delete":e, selectAll:e, removeFormat:e, unlink:e, insertHorizontalRule:e, bold:n, italic:n, underline:n, strikethrough:n, subscript:n, superscript:n, "|":function() {
    return new m({setEditor:function(a) {
      this.editor = a;
      this.button = new D({ownerDocument:a.ownerDocument});
    }});
  }});
  return q;
});
