...
k, x, p, y, z, A, q, B) {
  l = g("dijit.layout.ScrollingTabController", [x, y], {baseClass:"dijitTabController dijitScrollingTabController", templateString:l, useMenu:!0, useSlider:!0, tabStripClass:"", _minScroll:5, _setClassAttr:{node:"containerNode", type:"class"}, buildRendering:function() {
    this.inherited(arguments);
    var a = this.domNode;
    this.scrollNode = this.tablistWrapper;
    this._initButtons();
    this.tabStripClass || (this.tabStripClass = "dijitTabContainer" + this.tabPosition.charAt(0).toUpperCase() + this.tabPosition.substr(1).replace(/-.*/, "") + "None", h.add(a, "tabStrip-disabled"));
    ...
    this._buttons = v("\x3e .tabStripButton", this.domNode).filter(function(a) {
      if (this.useMenu && a == this._menuBtn.domNode || this.useSlider && (a == this._rightBtn.domNode || a == this._leftBtn.domNode)) {
        return this._btnWidth += m.getMarginSize(a).w, !0;
      }
    ...
    var b = this._contentBox = p.marginBox2contentBox(this.domNode, {h:0, w:a.w});
    ...
    p.layoutChildren(this.domNode, this._contentBox, [this._menuBtn, this._leftBtn, this._rightBtn, {domNode:this.scrollNode, region:"center"}]);
    ...
    var a = this.getChildren(), b = c.get(this.scrollNode, "width"), e = c.get(this.containerNode, "width") - b, d = this._getTabsWidth();
    if (a.length && d > b) {
      return {min:this.isLeftToRight() ? 0 : a[a.length - 1].domNode.offsetLeft, max:this.isLeftToRight() ? a[a.length - 1].domNode.offsetLeft + a[a.length - 1].domNode.offsetWidth - b : e};
    }
    a = this.isLeftToRight() ? 0 : e;
    return {min:a, max:a};
  }, _getScrollForSelectedTab:function() {
    var a = this._selectedTab, b = c.get(this.scrollNode, "width"), e = this._getScrollBounds(), a = a.offsetLeft + c.get(a, "width") / 2 - b / 2;
    ...
    var e = this, d = this.scrollNode, c = new n.Animation({beforeBegin:function() {
      this.curve && delete this.curve;
      var b = d.scrollLeft, g = e._convertToScrollLeft(a);
      c.curve = new n._Line(b, g);
    ...
    for (a = a.target;a && !h.contains(a, "tabStripButton");) {
      a = a.parentNode;
    }
    return a;
    ...
    if (!b || !h.contains(b, "dijitTabDisabled")) {
      var e = .75 * c.get(this.scrollNode, "width") * a, e = this._getScroll() + e;
  ...
  k = g("dijit.layout._ScrollingTabControllerButtonMixin", null, {baseClass:"dijitTab tabStripButton", templateString:k, tabIndex:"", isFocusable:function() {
    return !1;
  }});
  g("dijit.layout._ScrollingTabControllerButton", [q, k]);
  g("dijit.layout._ScrollingTabControllerMenuButton", [q, B, k], {containerId:"", tabIndex:"-1", isLoaded:function() {
    return !1;
  }, loadDropDown:function(a) {
    this.dropDown = new z({id:this.containerId + "_menu", ownerDocument:this.ownerDocument, dir:this.dir, lang:this.lang, textDir:this.textDir});
    var b = w.byId(this.containerId);
    r.forEach(b.getChildren(), function(a) {
      var c = new A({id:a.id + "_stcMi", label:a.title, iconClass:a.iconClass, disabled:a.disabled, ownerDocument:this.ownerDocument, dir:a.dir, lang:a.lang, textDir:a.textDir || b.textDir, onClick:function() {
        b.selectChild(a);
      }});
