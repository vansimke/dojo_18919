...
define("dijit/Menu", "require dojo/_base/array dojo/_base/declare dojo/dom dojo/dom-attr dojo/dom-geometry dojo/dom-style dojo/keys dojo/_base/lang dojo/on dojo/sniff dojo/_base/window dojo/window ./popup ./DropDownMenu dojo/ready".split(" "), function(w, k, x, p, g, q, t, u, v, l, f, h, y, r, z, A) {
  ...
  return x("dijit.Menu", z, {constructor:function() {
    this._bindings = [];
    ...
    var c = v.hitch(this, function(a) {
      ...
      return [l(a, c(this.leftClickToOpen ? "click" : "contextmenu"), function(a) {
        a.stopPropagation();
        a.preventDefault();
        (new Date).getTime() < d._lastKeyDown + 500 || d._scheduleOpen(this, e, {x:a.pageX, y:a.pageY}, a.target);
      }), l(a, c("keydown"), function(a) {
        if (93 == a.keyCode || a.shiftKey && a.keyCode == u.F10 || d.leftClickToOpen && a.keyCode == u.SPACE) {
          a.stopPropagation(), a.preventDefault(), d._scheduleOpen(this, e, null, a.target), d._lastKeyDown = (new Date).getTime();
        }
      })];
    });
    b.connects = d ? c(d) : [];
    ...
    if (d && g.has(d, a)) {
      for (var e = g.get(d, a) - 1, b = this._bindings[e], c;c = b.connects.pop();) {
        c.remove();
      }
    ...
    if (c) {
      if (b) {
        a = q.position(b, !0);
        var e = this._iframeContentWindow(b), e = q.docScroll(e.document), m = t.getComputedStyle(b), n = t.toPixelValue, l = (f("ie") && f("quirks") ? 0 : n(b, m.paddingLeft)) + (f("ie") && f("quirks") ? n(b, m.borderLeftWidth) : 0), b = (f("ie") && f("quirks") ? 0 : n(b, m.paddingTop)) + (f("ie") && f("quirks") ? n(b, m.borderTopWidth) : 0);
        c.x += a.x + l - e.x;
        c.y += a.y + b - e.y;
      }
    } else {
